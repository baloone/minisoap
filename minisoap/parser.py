...
class Sequence:
    def __init__(self, a1, a2=None):
        self.type = 'assign' if a2 is not None else 'call'
        self.variable_name = a1 if a2 is not None else None
        self.expr = a2 if a2 is not None else a1

    def __str__(self):
        return 'Sequence(' + self.variable_name.__str__() + \
            ', ' + self.expr.__str__() + ')'
...
class Transition(Expr):
    ...
    def __str__(self):
        return 'Transition(' + "; ".join(
            [str(t[0]) + " ->> " + str(t[1]) for t in self.table]
        ) + ')'
...
class Parser:
    ...
    def __init__(self):
        self._w = [chr(i) for i in range(65, 91)] + \
            [chr(i) for i in range(97, 123)] + ['\'', '_', "$"]
    ...
    def parse_line(self, line, continueparsing=None):
        if continueparsing is not None:
            vn = continueparsing.vn
            if vn is None:
                e = self.parse_expr(line, continueparsing)
                return Sequence(e) if e is not None else None
            else:
                return Sequence(vn, self.parse_expr(line, continueparsing))
        li = line[:(line.index('//'))] if '//' in line else line
        cur = 0
        while cur < len(li) and li[cur] in self._wh:
            cur += 1
        if cur >= len(li):
            return None
        if li[cur] in self._w:
            name = ''
            while cur < len(li)-1 and li[cur] in self._wn:
                name += li[cur]
                cur += 1
            while cur < len(li)-1 and li[cur] in self._wh:
                cur += 1
            if li[cur] == "=":
                try:
                    return Sequence(
                        VariableName(name),
                        self.parse_expr(li[cur+1:])
                    )
                ...
                for i in li[cur+1:]:
                    if i not in self._wh:
                        break
                else:
                    return Help(name)
        e = self.parse_expr(li)
        return Sequence(e) if e is not None else None
    ...
    def parse_expr(self, t, continueparsing=None):
        cur = 0
        funcs = [getattr(self, f) for f in dir(Parser)
                 if f.startswith("_"+self.__class__.__name__)]
        pile = []
        try:
            if continueparsing is not None:
            ...
            while cur < len(t):
                ...
                for f in funcs:
                    ...
                    raise LineParsingError(
                        'Unexpected character: '+t[cur-1:cur+1], cur)
                cur = self._skip_white_space(t, cur)
        except ContinueParsing as cp:
            cp.pile = pile
            raise cp
        ...
        try:
            ...
            for l in txt.split(";"):
                if l.replace(' ', '').replace('\t', '') == "":
                    continue
                [time, amplitude] = [i.replace(' ', '').replace(
                    '\t', '') for i in l.split(':')]
                if time[-2:] == "ms":
                    table.append((float(time[:-2])/1000, float(amplitude)))
                else:
                    table.append((float(time[:-1]), float(amplitude)))
    ...
    def __get_string(self, t, i):
        ...
        while t[cur+1] != '"':
            ...
            s += t[cur]
        cur += 2
        if cur < len(t) and not t[cur] in self._wh:
            raise LineParsingError('Expected whitespace after string', cur)
        return String(s), cur

    # Parses a variable name and returns the new cursor position
    def __get_variable_name(self, t, i):
        cur = i
        if not t[cur] in self._w:
            return None
        vn = t[cur]
        ...
        if cur < len(t) and not t[cur] in self._wh:
            raise LineParsingError(
                'Expected whitespace after variable name', cur)
        ...
        try:
            while t[cur+1] != ')' or p:
                ...
                if t[cur+1] == ')':
                    p -= 1
                if p < 0:
                    raise LineParsingError('Unexpected parenthesis', cur)
                cur += 1
                _t += t[cur]
        except IndexError:
    ...
    def __get_brackets(self, t, i, continueparsing=None):
        brkts = '' if continueparsing is None else continueparsing.txt+' '
        cur = i
        if continueparsing is None:
            if t[cur] != '{':
                return None
            else:
                cur += 1
    ...
    def __get_number(self, t, i):
        cur = i
        comma = False
        if not t[cur] in self._n and not t[cur] == '.':
            return None
        a = t[cur]
        ...
        cur += 1
        if cur < len(t) and not t[cur] in self._wh:
            raise LineParsingError('Expected whitespace after number', cur)
