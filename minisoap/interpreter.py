...
from .parser import Help, Expr, String, Number, VariableName, Transition
...
class Interpreter:
    ...
    def __init__(self):
        ...
        self.builtins_names = [f for f in dir(Builtins) if callable(
            getattr(Builtins, f)) and not f.startswith("__")]
        self.variables = {}
    ...
    def run(self, seq):
        ...
        if isinstance(seq, Help):
            ...
            if seq.variable_name.val in self.builtins_names:
                raise InterpreterError('Variable name not allowed')
            if seq.variable_name.val in self.variables:
                raise InterpreterError('Variable already defined')
    ...
    def run_expr(self, expr):
        if isinstance(expr, String) or isinstance(expr, Number):
            ...
            if expr.val not in self.variables:
                raise InterpreterError('Use of undefined variable: '+expr.val)
            return self.variables[expr.val]
        elif isinstance(expr, Expr):
            if not isinstance(expr.val, VariableName):
                raise InterpreterError('Cannot call a non callable')
            if expr.val.val not in self.builtins_names:
                raise InterpreterError('Unknown callable')
            return getattr(Builtins, expr.val.val)(
                self.builtins,
                *map(self.run_expr, filter(lambda x: x is not None, expr.args))
            )
        else:
            pass
