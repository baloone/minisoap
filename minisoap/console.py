...
class Console:
    ...
    def info(self, *args, **kw):
        ...
        Prints infos
        """
        self.log(*args, **kw, prefix=Back.BLUE + Fore.WHITE +
    ...
    def warn(self, *args, **kw):
        ...
        Prints warnings
        """
        self.log(*args, **kw, prefix=Back.YELLOW + Fore.BLACK +
    ...
    def error(self, *args, **kw):
        ...
        Prints errors
        """
        self.log(*args, **kw, prefix=Back.RED + Fore.WHITE +
                 'ERROR:' + Back.RESET + Fore.RESET + ' ')
    ...
    def input(self):
        ...
        def puts(x): return self.log(x, end='')

        def backspace_f(n=1):
            puts(Cursor.BACK(n)+self.line[self.cursor_pos:] +
                 ' '*n+Cursor.BACK(len(self.line)-self.cursor_pos+n))
            self.line = self.line[:self.cursor_pos-1] + \
        ...
        if kbhit():
            ...
            if c in ret:
                ...
                # right arrow
                elif c == M and self.cursor_pos < len(self.line):
                    self.cursor_pos += 1
                    puts(Cursor.FORWARD())

                elif c == H and len(self.hist_up) > 0:  # up arrow
                    puts(Cursor.BACK(
                        len(self.line[:self.cursor_pos])) +
                ...
                self.line = self.line[:self.cursor_pos] + \
                    c + self.line[self.cursor_pos:]
                puts(self.line[self.cursor_pos:] + Cursor.BACK(
                        len(self.line[self.cursor_pos:])
                    ) + Cursor.FORWARD())
                self.cursor_pos += 1
        return None
