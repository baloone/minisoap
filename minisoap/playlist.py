...
class Playlist(Stream):
    ...
    def __init__(self, dir_path, loop=False, transition=Transition()):
        ...
        for i in self.path.iterdir():
            if i.suffix[1:] in extensions:
                self.songs.append(i)

        self.duration = reduce(
            lambda x, y: x+y, [Song(i).duration for i in self.songs])
    # @var path
    ...
    def _charge(self, initial_call=False):
        if self._next is None and not initial_call:
            raise StopIteration
        if not initial_call:
            self._index += 1
        snd = None
        ...
        self._current = self._next if self._next is not None else Song(
            self.songs[self._index])
        self._next = snd
        self._ic = self._in if self._in is not None else iter(self._current)
        self._in = iter(self._next) if self._next is not None else None
    ...
    #
    def __next__(self):
        if self.songs == [] or self._ic is None:
            raise StopIteration
        try:
            ...
            if t <= 0:
                ...
                n1 = next(self._ic)
                n1 = np.concatenate(
                    (n1, np.zeros((self.chunk-len(n1), self.channels))))
                n2 = next(self._in) if self._in is not None else np.zeros(
                    (self.chunk, self.channels))
                n2 = np.concatenate(
                    (np.zeros((self.chunk-len(n2), self.channels)), n2))
                f = [np.vectorize(
                    lambda i: bar(
                        n1[i][j],
                        n2[i][j],
                        self._tr.amplitude(t+i*dt)
                    )
                ) for j in range(self.channels)]
                ret = np.transpose(np.array(
                    [f[i](np.arange(self.chunk)) for i in range(self.channels)]
                ))
            else:
                ret = next(self._ic)
            self.update_t()
